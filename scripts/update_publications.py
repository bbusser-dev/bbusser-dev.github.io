...
The script uses Benoit Busser's ORCID as the stable author identifier and
writes a Quarto/Markdown include plus a machine-readable JSON snapshot.
It uses only the Python standard library so GitHub Actions needs no extra
packages.
"""
...
def fetch_works() -> list[dict]:
    ...
    # Defensive de-duplication by OpenAlex work ID.
...
def make_markdown(works: list[dict]) -> str:
    lines = [
        "<!-- AUTO-GENERATED by scripts/update_publications.py. Do not edit manually. -->",
        "",
        f'<div class="publication-count"><strong>{len(works)} publications</strong> currently indexed for ORCID {esc(ORCID)} in OpenAlex.</div>',
        "",
    ]
...
def main() -> None:
    ...
    write_if_changed(OUT_MD, make_markdown(works))
    print(f"OpenAlex sync complete: {len(works)} works")
